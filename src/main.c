...
int main(int argc, char *argv[])
{
	...
	while (running) { 
		...
		if(mouse_timer == 0){
			...
			if (state.buttons & 1) {
				mouse_timer = 30;
				ALLEGRO_BITMAP *sub_bitmap1 = al_create_sub_bitmap(ImageCastle,0,0,al_get_bitmap_width(ImageCastle),al_get_bitmap_height(ImageCastle)/2 + 1);
				medianFilter(sub_bitmap1);
			}
		}
